...
DIFF_ATTR_TAG=['Label', 'Halo', 'Mark','Geometry', 'Graphic'];

//mapping from sld symbolizer til mapbox GL type-attribute
CONV_TYPE={ 'LineSymbolizer': 'line', 'PolygonSymbolizer':'fill', 'TextSymbolizer': 'symbol', 'PointSymbolizer':'symbol'}

//attributes that must be handeled different than the rest, and has relevant info placed differently (inner tag)
DIFF_ATTR=['stroke','opacity','fill', 'fill-opacity', 'font-size', 'stroke-width'];

//attrbiutes that belongs to the paint-object in Mapbox gl
PAINT_ATTR=['line-color', 'line-width','line-dasharray','line-opacity',
...
LAYOUT_ATTR=['text-field','text-font','text-max-size','text-max-witdth', 'line-join',
          'symbol-placement', 'icon-image'];

//mapping from sld to mapbox
CONVERT_ATTR_NAME={'stroke':'line-color',  'stroke-width':'line-width',  'stroke-dasharray':'line-dasharray','stroke-linejoin':'line-join','opacity':'line-opacity',
'PolygonSymbolizer-Fill-fill':'fill-color', 'PolygonSymbolizer-Fill-fill-opacity': 'fill-opacity', 'PolygonSymbolizer-Fill-opacity':'fill-opacity',
'font-size':'text-size', 'font-family':'text-font', 'Label':'text-field', 'TextSymbolizer-Halo-Fill-fill':'text-halo-color','TextSymbolizer-Fill-fill':'text-color'};

PUNKT='circle-12'; //?
//get the icon-image
CONV_ICON_IMG={'FKB_Gravplass.xml':'religious-christian-12', 'FKB_BautaStatue.xml':'monument-12','FKB_Bensinpumpe.xml':'fuel-24','FKB_Broenn.xml':'', 'FKB_Kum.xml':'circle-stroked-24'};//, 'FKB_MastTele':,'FKB_Mast_Telesmast': }
...
parseFile=function(data, file){
  ...
  parser.parseString(data, function (err, result) {
    FeatureTypeStyle=result.StyledLayerDescriptor.NamedLayer[0].UserStyle[0].FeatureTypeStyle;
    var rulesArr=[];
    var k;
    var rules=[];
    for(k=0; k<FeatureTypeStyle.length; k++){ //some files had more than one FeatureTypeStyle
      rulesVer=(FeatureTypeStyle[k].Rule);
      var rule;
      for(rule=0; rule<rulesVer.length; rule++){
        rules.push(rulesVer[rule]); //pushes all rules-tag in different FeatureTypeStyle-tags to one array
      }
    }
    var j;
    var maxzoom;
    ...
    for(j=0;j<rules.length;j++){
      ...
      for(i=0; i<ruleArray.length; i++){
        if((VALID_SYMBOLIZERS.indexOf(ruleArray[i]))>-1){
          writeJSON(rule[ruleArray[i]], ruleArray[i], name, minzoom, maxzoom, file);//Sends object, symbolizer and filename
        }
      }
    }
  });
  ...
  try{
    ...
    if(cssObj['fill-color']!==undefined && cssObj['line-color']!== undefined){
      ...
      for(i=attPos; i<(size); i++){
        var key=Object.keys(cssObj)[attPos];//since i delete for each loop, it will always be this position
        obj[key]=cssObj[key];
        delete cssObj[key];
      }
      styleObj1=make_JSON(name, convType, cssObj, minzoom, maxzoom)
...
function make_JSON(name, type, cssObj, minzoom, maxzoom){//name=file name, css is an object [cssName: cssValue]pairs, cssName is ie stroke, stroke-width
...
function getSymbolizersObj(symbTag, type, file){
  ...
  var i;
  var cssObj={};
  for(i=0;i<Object.keys(symbTag[0]).length;i++){ //for alle tags under <-Symbolizer>
    var tagName=Object.keys(symbTag[0])[i];
    if(VALID_ATTR_TAGS.indexOf(tagName)>-1){  //hvis tag finnes i valid-array, eks Stroke
      if(DIFF_ATTR_TAG.indexOf(tagName)>-1 || ((tagName==='Fill') && symbTag[0].Fill[0].GraphicFill !== undefined)){ //if values are not in the regular place
        ...
        var cssArray= getCssParameters(symbTag, tagName, type); //array med key-value pair for aa legge inn i cssObj
        var k;
...
function getCssParameters(symbTag, validAttrTag, type, outerTag){
  ...
  var j;
  for(j=0; j<nrOfCssTags; j++){ //for all cssParametere
...
//gets called if attribute-values are not placed as the rest and therefor needs a different method the get the css-value
...
function getLabelObj(tagName, type, symbTag, obj){
  convertedTagName=convertCssName(tagName, tagName, type);
...
function getHaloObj(tagName, type, symbTag, obj){
  ...
  for(j=0;j<Object.keys(symbTag[0].Halo[0]).length;j++){
    ...
    if(innerTagName==='Radius'){
      ...
      obj['text-halo-width']=parseInt(value);
    }
    else if(innerTagName ==='Fill'){
      var cssArray= getCssParameters(symbTag, innerTagName, type,'Halo'); //array with key-value pair to add in obj
      var k;
      for(k=0;k<cssArray.length; k++){
...
function getGraphicObj(file, symbTag, type, obj){
  ...
  try{
    var size=symbTag[0].Graphic[0].Size[0];
      obj['icon-size']=parseInt(size);
  }
  catch(err){
      console.log('Size finnes ikke i denne graphic-taggen');
  }
...
function getIconImage(file){
  try{
    ...
    console.log('har ikke registrert ikon');
    img=undefined;
  }
  return img;
}
...
function convert_css_parameter(cssTag, ValidAttrTag, type, outerTag){
  ...
  try{
    ...
    if((DIFF_ATTR.indexOf(cssName))>-1 && !(regInt.test(cssTag['_'])) && !(regDouble.test(cssTag['_'])) && !regLetters.test(cssColorValue) &&!regNumbers.test(cssColorValue) ){//Check if different type of attribute
    ...
    if((DIFF_ATTR.indexOf(cssName))>-1 && !(regInt.test(cssTag['_'])) && !(regDouble.test(cssTag['_']))){//Check if different type of attribute
      cssValue= (cssTag['ogc:Function'][0]['ogc:Literal'][1]);
    }
    else{
      cssValue= cssTag['_'];
  ...
  convertedCssName = convertCssName(cssName, ValidAttrTag, type, outerTag);
  convertedCssValue = convertCssValue(cssValue,cssName);
  return [convertedCssName, convertedCssValue];
}

//Makes sure the attribute values are returned in the correct type and defined correctly (ie colors with a # in front)
function convertCssValue(cssValue,cssName){
  if((cssName==="stroke" || cssName==="stroke-linejoin" || cssName === "stroke-linecap")){ //linejoin describes rendering with values; mitre/round/bevel
    return "#"+cssValue.split('#')[cssValue.split('#').length-1];  //some colors are defined with #, others not. Split removes the # if it exists, so i always end up with the color value without the #                                                               //linecap is a line-border with values; butt/round/square
  }
  if(cssName==="stroke-width" || cssName === "stroke-opacity" || cssName === "stroke--dashoffset"){
  ...
  if(cssName==="fill"){
    return "#"+cssValue.split('#')[cssValue.split('#').length-1]; //some colors are defined with #, others not. Split removes the # if it exists, so i always end up with the color value without the #
  }
...
function convertCssName(cssName, validAttrTag, type, outerTag){
  var newName;
  if(cssName==='fill' || cssName==='fill-opacity' || cssName==='opacity' && validAttrTag==='Fill'){
    if(outerTag===undefined){
      ...
      if(newName===undefined){
        console.log("could not convert the attribute name: "+type+'-'+outerTag+'-'+validAttrTag+'-'+cssName);
      }
    }
    return newName;
  }
    ...
    ACCEPTED=['font-weight','font-style'];    //List to print those I know cannot be translated
    if(newName===undefined && ACCEPTED.indexOf(newName)>-1){ //scip printing the ones I know are not translated
